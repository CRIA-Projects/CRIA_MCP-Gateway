...
let target, deviceToken;
async function api(path, body, method = "POST", identity) {
  const response = await fetch(target + path, { method: body === undefined ? "GET" : method, headers: { "content-type": "application/json", "x-admin-key": key, ...(identity ? { authorization: `Bearer ${deviceToken}`, "x-client-id": "forged-user" } : {}) }, ...(body === undefined ? {} : { body: JSON.stringify(body) }), signal: AbortSignal.timeout(10000) });
  assert.ok(response.ok || (path === "/mcp" && !identity && response.status === 401), `${path}: HTTP ${response.status}`);
...
try {
  ...
  deviceToken = (await api("/admin/credentials", { userId: "smoke", deviceName: "Docker test" })).token;
