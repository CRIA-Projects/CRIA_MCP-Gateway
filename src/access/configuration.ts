...
export interface McpServer { id: string; name: string; description: string; kind: McpServerKind; endpoint?: string; authorizationHeader?: string; }
...
export class PersistentAccessConfiguration implements AccessConfiguration {
  ...
  async createMcpServer(input: McpServer) { validateServer(input); return this.mutate((state) => { if (state.mcpServers.some((server) => server.id === input.id)) throw new Error("An MCP server with this ID already exists"); const server = copy(input); if (!server.authorizationHeader) delete server.authorizationHeader; state.mcpServers.push(server); return copy(server); }); }
  async updateMcpServer(id: string, input: Omit<McpServer, "id">) { validateServer({ id, ...input }); return this.mutate((state) => { const server = state.mcpServers.find((candidate) => candidate.id === id); if (!server) return undefined; Object.assign(server, input); if (input.kind === "demo") { delete server.endpoint; delete server.authorizationHeader; } else { server.endpoint = input.endpoint; if (input.authorizationHeader) server.authorizationHeader = input.authorizationHeader; else delete server.authorizationHeader; } return copy(server); }); }
