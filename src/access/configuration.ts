...
export interface McpServer { id: string; name: string; description?: string; kind: McpServerKind; endpoint?: string; authorizationHeader?: string; authHeaderName?: string; }
...
export class PersistentAccessConfiguration implements AccessConfiguration {
  ...
  async createMcpServer(input: McpServer) { validateServer(input); return this.mutate((state) => { if (state.mcpServers.some((server) => server.id === input.id)) throw new Error("An MCP server with this ID already exists"); const server = copy(input); if (!server.authorizationHeader) { delete server.authorizationHeader; delete server.authHeaderName; } state.mcpServers.push(server); return copy(server); }); }
...
function validateServer(server: McpServer) { if (!idPattern.test(server.id) || !server.name.trim()) throw new Error("MCP server requires a lowercase ID and a name"); if (server.kind === "demo" && (server.endpoint || server.authorizationHeader || server.authHeaderName)) throw new Error("A demo MCP server cannot have an endpoint or authentication"); if (server.kind === "remote") { if (!server.endpoint) throw new Error("A remote MCP server requires an HTTPS endpoint"); if (new URL(server.endpoint).protocol !== "https:") throw new Error("A remote MCP endpoint must use HTTPS"); if (server.authHeaderName && !headerNamePattern.test(server.authHeaderName)) throw new Error("The authentication header name is not a valid HTTP header name"); } }
